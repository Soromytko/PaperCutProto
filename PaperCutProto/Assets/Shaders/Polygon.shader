        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                if (_Progress > 0)
                {
                    ...
                    vertex = vertex * (1.0 - _Progress) + vertex * pow(1 - per, 1) * _Progress;
                    float height = tex2Dlod(_HeightMap, float4(v.uv.xy, 0, 0)).r;
                    vertex.y = height * _VerticalNoiseForce * _Progress;

                    vertex.x += rand(-1, 1, sin(height)) * _HorizontalNoiseForce * (1.0 - height) * _Progress;
                    vertex.z += rand(-1, 1, cos(height)) * _HorizontalNoiseForce * (1.0 - height) * _Progress;
